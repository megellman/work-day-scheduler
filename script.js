...
$(function () {
    ...
    var hour;
    for (var i = 9; i < 18; i++) {
        hour = (dayjs().hour(i).format('H'));
        var timeBlockDiv = $('<div>').addClass('row time-block future').attr('id', hour);
        ...
        sampleTime = Number(dayjs().format('H'));
        if (hour.attr('id') < sampleTime) {
            timeBlockDiv.removeClass('future present');
            timeBlockDiv.addClass('past');
        } else if (hour.attr('id') > sampleTime) {
